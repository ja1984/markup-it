import { Text, Mark, Block, Document, Inline } from 'slate';
export { Block, Document, Inline, Leaf, Mark, Text } from 'slate';
import Immutable, { Record, List, Stack, Set, Map, OrderedMap } from 'immutable';
import uid from 'uid';
import is from 'is';
...
import splitLines$1 from 'split-lines';
...
class Serializer extends RuleFunction {
  ...
  transformText(transform) {
    ...
    .transformLeaves((state, inputLeaf) => {
      ...
      return leaf.merge({
        marks: leaf.marks.add(Mark.create({
          type: MARK
        }))
      });
    });
...
class State extends Record(DEFAULTS$1) {
  /**
  ...
  deserializeToDocument(text) {
    const document = this.use('document').deserialize(text).get(0) || Document.create();
    let {
      nodes
...
function selectInlines(node) {
  ...
  return nodes.reduce((result, child) => result.concat(selectInlines(child)), List());
}
/**
...
function parse(str) {
  ...
  const root = Document.create({}); // The top of the stack always hold the current parent
  // node. Should never be empty.

  let stack = Stack().push(root); // The current marks
...
const serialize$k = SerializerFactory().matchObject('document').then(state => {
  ...
  const prettified = prettyPrint(text, {
    indent_size: 2
  });
...
function deserializeCodeLines(text) {
  ...
  return lines.map(line => Block.create({
    type: BLOCKS.CODE_LINE,
    nodes: [Text.create(line)]
  }));
}
...
const REPLACEMENTS_ESCAPE = Map([['*', '\\*'], ['#', '\\#'], // GitHub doesn't escape slashes, and render the backslash in that cause
// [ '/', '\\/' ],
...
const serialize$t = SerializerFactory().matchType(BLOCKS.TABLE).then(state => {
  ...
  if (mustSerializeAsHTML(table)) {
    ...
    const htmlOutput = htmlState.serializeDocument(Document.create({
      nodes: [table]
    }));
...
const REPLACEMENTS = OrderedMap([['\\', '\\\\'], ['*', '\\*'], ['#', '\\#'], ['(', '\\('], [')', '\\)'], ['[', '\\['], [']', '\\]'], ['`', '\\`'], ['_', '\\_'], ['|', '\\|'], ['"', '\\"'], ["'", "\\'"]]);
...
const serialize$y = SerializerFactory().matchType(isCustomType).then(state => {
  ...
  const innerNodes = containsInline ? List([wrapInDefaultBlock(node.nodes)]) : node.nodes;
...
export { BLOCKS, CONTAINERS, DeserializerFactory as Deserializer, HTMLParser, INLINES, LEAFS, MARKS, MarkdownParser, SerializerFactory as Serializer, State, TABLE_ALIGN, VOID };
